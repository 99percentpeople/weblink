...
import { RtcProtocolError } from "./errors";
  ...
  if (
    !optional(
      ...
      (status) =>
        ...
        ["sending", "received", "error"].includes(status),
    )
  )
    return fail();
  let valid = false;
...
export function parseSessionMessage(
  ...
  if (typeof raw !== "string")
    throw new RtcProtocolError(
      "invalid-message",
      "RTC control messages must be JSON strings",
    );
  try {
    return validateSessionMessage(JSON.parse(raw));
  } catch (error) {
    if (error instanceof RtcProtocolError) throw error;
    throw new RtcProtocolError(
      "invalid-message",
      "Invalid RTC message JSON",
    );
  }
}
...
export function assertMessagePeer(
  ...
  if (
    ...
    throw new RtcProtocolError(
      "invalid-message",
      "RTC message does not match its owning session",
    );
  }
}
...
>(message: T): T {
  ...
  try {
    ...
    throw new RtcProtocolError(
      "invalid-message",
      "RTC messages must be JSON serializable",
    );
  }
