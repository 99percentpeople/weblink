...
import type { FileTransferer } from "../core/file-transferer";
...
export function fileTasks(
  ...
  // Only the newest message for a file owns its live transfer. Earlier history
  // must not become active again when the same cached file is resent.
  const newest = new Map<string, FileTransferMessage>();
  for (const message of messages) {
    const previous = newest.get(message.fid!);
    if (
      !previous ||
      previous.createdAt <= message.createdAt
    )
      newest.set(message.fid!, message);
  }
  return messages.map((message) => {
    const outgoing = message.client === self;
    const cache = caches[message.fid!];
    const live =
      newest.get(message.fid!) === message &&
      !!transfers[message.fid!];
